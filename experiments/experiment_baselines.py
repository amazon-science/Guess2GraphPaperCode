"""
Experiment Baselines Module

Run causal discovery experiments with different parameter combinations.
"""
...
def _trial_worker(args):
    ...
    dimensionality, sample_size, edge_probability, algorithms, metrics_to_compute, trial_dir = args
    try:
        return run_single_trial(dimensionality, sample_size, edge_probability, algorithms, metrics_to_compute, trial_dir)
...
def run_single_trial(
    ...
    Args:
        ...
        edge_probability: Probability of edge existence (0-1)
    ...
    for algorithm in algorithms:
        predicted_skeleton, runtime = run_method(algorithm, data)
...
def print_experiment_summary(exp_dir: str, config: Dict[str, Any]):
    ...
    for dim, sample_size, edge_probability in combinations:
        ...
        print(f"\nDim={dim}, Samples={sample_size}, Edge_Probability={edge_probability}:")
        print("-" * 50)
...
def run_experiment(config: Dict[str, Any], results_dir: str = None) -> str:
    ...
    for dim, sample_size, edge_probability in combo_pbar:
        ...
        for trial_idx in range(config['num_trials']):
            ...
            trial_args.append((dim, sample_size, edge_probability, config['algorithms'], config['metrics'], trial_dir))
...
if __name__ == "__main__":
    config = {
        ...
        'graph_params': {
            ...
            'edge_probabilities': [0.2]  # Probability of edge existence
        },
